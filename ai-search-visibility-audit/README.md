...
2. Paste at least one API key. **Cheapest option: a free Google AI Studio
   key** — select the Gemini engine and the audit runs entirely on Gemini's
   free tier (questions are generated by Gemini too). Add an OpenAI key for
   the ChatGPT engine and a Perplexity key for Perplexity.
3. Keep 30 prompts × 3 samples for a first audit (about 90 answers per engine;
   free on Gemini's free tier, typically under $1 of OpenAI usage with
   `gpt-4o-mini`). Free-tier rate limits are waited out automatically, so a
   Gemini-only audit takes a few minutes longer.
...
| `geminiApiKey` / `geminiModel` | – / `gemini-2.5-flash` | Gemini engine (Google Search grounding); generates the questions when there is no OpenAI key. Free AI Studio key works. |
...
billed by those providers separately: free on Gemini's free tier, and
typically well under $1 per audit on OpenAI with the default settings.
...
**Can I run it without paying for an AI API?** Yes: a Google AI Studio key has a
free tier that covers a default audit (30 prompts × 3 samples). Select only the
Gemini engine and leave the other keys empty.
